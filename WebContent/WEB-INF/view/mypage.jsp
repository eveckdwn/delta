...
<form method="post" enctype="multipart/form-data" action="/updateUser">
	...
	<div align="center">
		...
		<p>
			<c:if test="${!empty ban }">
				<a style="color: red">제재 기간이 <b>${ban }</b>일 남았습니다.<br /> 제재
					기간동안에는 해당 서비스를 이용하실 수 없습니다.<br /> <fmt:formatDate var="d"
						value="${user.BAN}" pattern="yyyy년 MM월 dd일" /> 제재가 풀리는 날은 <b>${d }</b>
					입니다.
				</a>
			</c:if>
		</p>
		...
		<div>
			...
			<c:if test="${user.LV == 1 }">
				<small style="color: blue">이메일 인증을 받은 사용자입니다. 다양한 서비스를 이용할 수
					있습니다.</small>
			</c:if>
		</div>
		<div>
